...
export default function EditEstateVR({ isEdit }) {
    ...
    // }
    const addText = (addData, scene) => {
        addData.type = "text";
        addData.name = addData.data.substr(0, 10);//just get 10 character 
        const material = new THREE.MeshPhongMaterial({ color: 0xFFFF00 });
        //Add object
        const loader = new FontLoader();
        const font = loader.parse(fontHelve);
        const geometry = new TextGeometry(addData?.data ?? "MetaEarth", {
            font: font,
            size: 5,
            height: 1,
            curveSegments: 1,
        ...
        doAddMesh({ object: textMesh, addData, scene });
    }
    const ChangeSelectMesh = (selectMesh) => {
        //Restore color
        const box = new THREE.Box3();
        box.setFromObject(selectMesh);
        if (boundingBox) boundingBox.removeFromParent();
        let helper = new THREE.Box3Helper(box, 0xffff00);
        setBoundingBox(helper);
        threeLayer.scene.add(helper);
        setCurrentObj(selectMesh)
    }
        ...
        try {
            ...
            if(!vrcontent.content){
                ...
                vrcontent.content.forEach((obj) => {
                    switch (obj.type) {
                        case "text": addText(obj, scene); break;
                        case "3d": add3d(obj, scene); break;
                        default: break;
                    }
                })
                let checkIntervalId = setInterval(() => {
                    let newListObj = scene.children.filter(e => e.addData?.type === "text" || e.addData?.type === "3d");
                    if (newListObj.length === vrcontent[0].content.length) {
            ...
            <Center className='loaded' textAlign="center" height="calc(100vh - 80px)" position="relative" >
                ...
                {isEdit ? <>
                    <CollapsPanel title="Action" left="10px" top="10px">
                        ...
                        <Flex m="2" border={'2px'} borderColor="white" rounded="md" p="2">
                            <Button colorScheme="green" mx="1" size="xs" onClick={() => setAddType("text")}>Add text</Button>
                            <Button colorScheme="green" mx="1" size="xs" onClick={() => setAddType("3d")}>Add 3D Model</Button>
                        </Flex>
                        {addType === "text" ? <Flex m="2" direction="column">
                            <Input color="white" size="sm" placeholder='Fill text' onChange={e => setAddData({ data: e.target.value })} rounded="md" />
                            <Button colorScheme="green" m="2" size="xs" onClick={() => { setIsLoadingAddObj(true); addText(addData); setAddType(""); }}>Add text</Button>
                        </Flex> : null}
                        {addType === "3d" ? <Flex m="2" direction="column">
                            <Input color="white" size="sm" placeholder='Fill url to 3d file' onChange={e => setAddData({ data: e.target.value })} rounded="md" />
                            <Button colorScheme="green" m="2" size="xs" onClick={() => { add3d(addData); setAddType("") }}>Load from url</Button>
                            <IpfsFileUploader colorScheme="green" m="2" size="xs" accept={".fbx,.gltf,.jpg,.mtl,.png,.bin"} result={handlerLoad3DLocalResult} onClick={() => setIsLoadingAddObj(true)} onCancel={() => setIsLoadingAddObj(false)}>Add 3d file</IpfsFileUploader>
                    ...
                    <CollapsPanel title="List object" bottom="10px" left="10px" width="200px">
                        {
                            listObj && listObj.map((e, index) =>
                                <Button size="xs" key={index} leftIcon={<BsBox />} onClick={() => ChangeSelectMesh(e)} my="1" fontWeight={currentObj && currentObj.uuid === e.uuid ? 'bold' : 'light'}>[{e.addData.type.toUpperCase()} {e.id}] {e.addData.name}</Button>
                            )
                        }
                    </CollapsPanel>
